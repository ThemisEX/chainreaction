import { web3, DUST_AMOUNT, ONE_ALPH, prettifyAttoAlphAmount, number256ToBigint, number256ToNumber, NULL_CONTRACT_ADDRESS, sleep, ALPH_TOKEN_ID, MINIMAL_CONTRACT_DEPOSIT } from '@alephium/web3'
...
describe('integration tests', () => {
  ...
  it('start game with players playing', async () => {
    ...
    expect(state.asset.tokens).toEqual([])
      ...
      it('start game with players playing custom token, check burn', async () => {
    const signer = await testNodeWallet()
    ...
    expect(state.fields.endTimestamp).toBeLessThanOrEqual(now)
    expect((await game.view.canEnd()).returns).toBe(true)
    expect(state.fields.burnedAmount).toBeGreaterThan(0n)
